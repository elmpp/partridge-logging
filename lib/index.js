...
const myFormatWithDumpables = printf((info) => {
    if (info.dumpables) {
        return `${info.timestamp} ${info.runtime_label ? '[' + info.runtime_label + ']' : ''} ${info.level}: ${info.message} dumpables: ${util_1.default.inspect(info.dumpables || {}, { showHidden: false, depth: null })}`;
    }
    return `${info.timestamp} ${info.runtime_label ? '[' + info.runtime_label + ']' : ''} ${info.level}: ${info.message}`;
});
