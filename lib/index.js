...
const myFormat = winston_1.format.combine(label({
    message: false,
}), timestamp(), winston_1.format.errors({ stack: true }), printf((info) => {
...
const myFormatWithDumpables = winston_1.format.combine(winston_1.format.errors({ stack: true }), printf((info) => {
    let formatted = `${info.timestamp} ${info.runtime_label ? runtimeLabel(info.runtime_label) : ''} ${info.level}: ${info.message}\n${info.stack}`;
    if (info.stack) {
        formatted += `\n${info.stack}`;
    }
    if (info.dumpables) {
        formatted += `\n dumpables: ${util_1.default.inspect(info.dumpables || {})}`;
    }
    return formatted;
}));
...
if (partridge_config_1.config.logging.consoleEnable) {
    ...
    debug(`Logging: ${JSON.stringify({ transports: [...transports.keys()], level: partridge_config_1.config.logging.level })}`);
    transports.set('console', new winston_1.transports.Console({ format: myFormatWithDumpables, silent: true }));
}
