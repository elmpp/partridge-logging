    ...
    log(logLevelOrMessage, messageOrOptions, optionsArg) {
        ...
        try {
            if (messageOrOptions instanceof Error) {
                ...
                throw new dumpable_error_1.default('Unrecognised log calls', { logLevelOrMessage: [logLevelOrMessage], messageOrOptions: [messageOrOptions], options: [optionsArg] });
            }
            this.logProvider.log(logLevel, message, this.optionsReducer(options, logLevel));
        }
        catch (err) {
            this.logProvider.log('error', err);
        }
        return this;
    }
    end() {
